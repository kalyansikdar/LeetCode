...
class TrieTree:
    ...
    def longestPrefix(self):
        ...
        while not parent.isWord and len(parent.children) == 1:
            # take the first child, we do not need other children as we break
            # out of while loop if that node has more children
...
solution = Solution()
sentence = ["leet", "leetcode", "leets", "leeds"]
result = solution.longestCommonPrefix(sentence)
print(result)
# Note: It has an implementation using Trie as well
